...
A naive nearest-neighbor search in a cloud containing millions of points is comes at a heavy computational cost, especially considering that the point cloud often contains millions or even billions of points. Efficient spatial data structures such as KD-trees enables much more efficient searching
...
The overall structure of the implementation follows a sequence of steps. First, the LiDAR file is read and all points are stored in a contiguous array. Next, a KD-tree is constructed directly within this array to enable efficient nearest-neighbor queries. For each point, the KD-tree is queried to find its k closest neighbors. Finally, a local covariance matrix is assembled from these neighbors, and its eigenvectors are computed to determine the surface normal at the point.

\subsection{Reading the LiDAR file}
The first step of the project is to convert the LiDAR file into an internal data structure suitable for efficient access. Although the point coordinates could be read directly from the file every time a point's position is needed, repeatedly decoding individual entries would introduce significant overhead, not worth the memory savings.
Instead, the entire file is read once, and each point is stored in an array of custom point structures containing the (x,y,z) coordinates.
...
\subsection{Fining the surface normals}
For each point, a surface normal is estimated by using its nearest neighbors. Given the $k$ nearest neighbors, a $3\times3$ covariance matrix is constructed. The surface normal is then defined as the eigenvector corresponding to the smallest eigenvalue of this matrix (see ???). The eigenvalue decomposition of the symmetric $3\times3$ matrix is done using GNU Scientific Library (GSL).

Since eigenvectors are only defined up to sign, the computed normals generally point in inconsistent directions across the point cloud. To obtain a consistent orientation, a neighborhood graph is constructed where edges connect nearby points. Starting from a reference point, the orientation of the neighboring normals are flipped when necessary, such that the neighboring normals point in the same general direction. This is then repeated for the neighborhoods of the points just treated, until the orientation of the normals are coherent over the entire surface.
